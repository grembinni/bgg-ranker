...
describe('selectRandomPair (RANK-02)', () => {
  ...
  it('excludes games with rating >= 850 or <= 150 from eligible pool', () => {
    // g0=900 (excluded high), g1=151 (eligible), g2=849 (eligible), g3=150 (excluded low)
    const ratings = { g0: 900, g1: 500, g2: 400, g3: 100 }
    for (let k = 0; k < 50; k++) {
      const pair = selectRandomPair(ratings, [])!
      expect(pair).not.toContain('g0')
      expect(pair).not.toContain('g3')
    }
  })

  it('falls back to full set when fewer than 2 games are eligible', () => {
    // All games outside eligible range — must still return a pair
    const ratings = { g0: 900, g1: 850, g2: 150 }
    const pair = selectRandomPair(ratings, [])
    expect(pair).not.toBeNull()
    expect(pair![0]).not.toBe(pair![1])
  })
  ...
  it('only pairs games within 50 rank positions', () => {
    // 80 games in eligible range (200–840); all pairs must be within 50 ranks
    const ratings: Record<string, number> = {}
    for (let i = 0; i < 80; i++) ratings[`g${i}`] = 840 - i * 8  // 840 down to 216
    for (let k = 0; k < 50; k++) {
      const pair = selectRandomPair(ratings, [])!
  ...
  it('pairs within ±1 tier when a valid partner exists', () => {
    // 10 games in tiers 4-7 (ratings 310-700), all eligible and within 50 ranks
    const ratings: Record<string, number> = {}
    for (let i = 0; i < 10; i++) ratings[`g${i}`] = 700 - i * 40  // 700 down to 340
    for (let k = 0; k < 30; k++) {
      const pair = selectRandomPair(ratings, [])!
